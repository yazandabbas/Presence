  ...
  it("creates findings, follow-up proposals, and repo projections for Presence memory", async () => {
    ...
    try {
      await fs.writeFile(
        ...
        JSON.stringify(
          { name: "presence-projections", scripts: { test: 'node -e "process.exit(0)"' } },
          null,
          2,
        ),
        "utf8",
      ...
      await system.presence
        ...
        .pipe(Effect.runPromise);
      const proposal = await system.presence
        ...
        .pipe(Effect.runPromise);
      const childTicket = await system.presence
        .materializeFollowUp({
      ...
      await waitFor(
        async () =>
          ...
          (await fs.readFile(progressPath, "utf8")).includes(
            "Captured the current execution state",
          ) &&
      ...
      expect(
        existsSync(path.join(projectionRoot, "tickets", ticket.id, "current_summary.md")),
      ).toBe(true);
      expect(
        existsSync(
      ...
      expect(
        await fs.readFile(path.join(projectionRoot, "tickets", ticket.id, "ticket.md"), "utf8"),
      ).toContain(ticket.title);
      expect(await fs.readFile(progressPath, "utf8")).toContain(
        "Captured the current execution state",
      );
      expect(
        await fs.readFile(
          path.join(projectionRoot, "brain", "runbooks", "projection-coverage.md"),
          "utf8",
        ),
      ...
      const snapshot = await system.presence
        ...
        .pipe(Effect.runPromise);
      expect(
        snapshot.ticketSummaries.find((summary) => summary.ticketId === ticket.id),
      ).toBeTruthy();
      expect(
        snapshot.proposedFollowUps.find((candidate) => candidate.id === proposal.id)
          ?.createdTicketId,
      ).toBe(childTicket.id);
    } finally {
