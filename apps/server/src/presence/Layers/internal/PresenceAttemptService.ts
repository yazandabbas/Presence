...
import { latestToolWorkerHandoffForThread } from "./PresenceToolBridge.ts";
...
type PresenceAttemptServiceDeps = Readonly<{
  ...
  readAvailableProviders: () => Effect.Effect<
  ...
  upsertOperationLedger: (input: {
    ...
    phase: PresenceOperationRecord["phase"];
    status: PresenceOperationRecord["status"];
    dedupeKey: string;
    summary: string;
    details?: Record<string, unknown> | undefined;
    counters?: ReadonlyArray<{ name: string; value: number }> | undefined;
    error?: PresenceOperationRecord["error"] | undefined;
  ...
  evaluateSupervisorActionInternal: (input: {
    ...
    ticketId: string;
    attemptId?: string | null;
  }) => Effect.Effect<{ allowed: boolean; reasons: ReadonlyArray<string> }, Error, never>;
  ...
  mapAttempt: (
    row: AttemptRow,
  ) => PresenceControlPlaneShape["attachThreadToAttempt"] extends (
    ...args: never[]
  ) => Effect.Effect<infer A, Error, never>
    ? A
    : never;
  encodeJson: (value: unknown) => string;
  markTicketEvidenceChecklist: (ticketId: string) => Effect.Effect<void, Error, never>;
  ...
  ) => Effect.Effect<ParsedPresenceHandoffBlock | null, Error, never>;
...
const makePresenceAttemptService = (deps: PresenceAttemptServiceDeps): PresenceAttemptService => {
  ...
  const buildWorkerHandoffCandidate: PresenceAttemptService["buildWorkerHandoffCandidate"] = (
    input,
  ) =>
    Effect.gen(function* () {
      const latestToolHandoff =
        input.thread?.id != null
          ? latestToolWorkerHandoffForThread(input.missionEvents, {
              threadId: input.thread.id,
              attemptId: input.attemptId,
            })
          : null;
      const latestAssistantReasoning = yield* deps.readLatestAssistantReasoningFromThread(
        input.thread,
      );
      const previousReasoningUpdatedAt = input.previousHandoff?.reasoningUpdatedAt ?? null;
      const useToolReasoning =
        latestToolHandoff &&
        (!previousReasoningUpdatedAt ||
          latestToolHandoff.updatedAt.localeCompare(previousReasoningUpdatedAt) >= 0);
      ...
      const reasoningCompletedWork = useToolReasoning
        ? latestToolHandoff.completedWork
        : useAssistantReasoning
          ? latestAssistantReasoning.completedWork
          : (input.previousHandoff?.completedWork ?? []);
      const reasoningCurrentHypothesis = useToolReasoning
        ? latestToolHandoff.currentHypothesis
        : useAssistantReasoning
          ? latestAssistantReasoning.currentHypothesis
          : (input.previousHandoff?.currentHypothesis ?? null);
      const reasoningNextStep = useToolReasoning
        ? latestToolHandoff.nextStep
        : useAssistantReasoning
          ? latestAssistantReasoning.nextStep
          : (input.previousHandoff?.nextStep ?? null);
      const reasoningOpenQuestions = useToolReasoning
        ? latestToolHandoff.openQuestions
        : useAssistantReasoning
          ? latestAssistantReasoning.openQuestions
          : (input.previousHandoff?.openQuestions ?? []);
      const reasoningSource = useToolReasoning
        ? latestToolHandoff.source
        : useAssistantReasoning
          ? latestAssistantReasoning.source
          : (input.previousHandoff?.reasoningSource ?? null);
      const reasoningUpdatedAt = useToolReasoning
        ? latestToolHandoff.updatedAt
        : useAssistantReasoning
      ...
      const testsRun = deps.uniqueStrings([
        ...
        ...(useToolReasoning ? latestToolHandoff.testsRun : []),
      ]);
      ...
      const blockers = deps.uniqueStrings([
        ...(useToolReasoning ? latestToolHandoff.blockers : []),
  ...
  const synthesizeWorkerHandoffFromThread: PresenceAttemptService["synthesizeWorkerHandoffFromThread"] =
    (attemptId, options) =>
      Effect.gen(function* () {
        ...
        const [thread, previousHandoff, changedFiles, findings, missionEvents] = (yield* Effect.all(
          [
            ...
            deps.readRecentMissionEventsForBoard(context.boardId),
          ],
        )) as [
          {
            id: string;
        ...
        const nextHandoff = yield* buildWorkerHandoffCandidate({
          ...
          findings: findings.filter(
            (finding) => finding.attemptId === null || finding.attemptId === attemptId,
          ),
          missionEvents,
        });
  ...
  const normalizeCreateAttemptError = (ticketId: string, cause: unknown): PresenceRpcError =>
    deps.isPresenceRpcError(cause)
      ...
      : deps.isSqliteUniqueConstraintError(cause)
        ...
        : deps.presenceError("Presence could not open the worker path.", cause);
  ...
  const createAttempt: PresenceAttemptService["createAttempt"] = (input) =>
    ...
    }).pipe(Effect.mapError((cause) => normalizeCreateAttemptError(input.ticketId, cause)));
  ...
  const service: PresenceAttemptService = {
    ...
    cleanupWorkspace: (input) =>
      Effect.gen(function* () {
        ...
        yield* deps.upsertOperationLedger({
          ...
          phase: "finish",
          status: nextAttemptStatus === "interrupted" ? "cancelled" : "completed",
          dedupeKey: operationWorkspaceCleanupKey(context.attemptId),
          summary:
            nextAttemptStatus === "interrupted"
              ? "Workspace cleanup interrupted the attempt."
              : "Workspace cleanup completed.",
          details: {
            cleanupWorktreeDone: Boolean(context.workspaceWorktreePath),
            cleanupThreadDone: Boolean(context.attemptThreadId),
          },
          counters: [
            {
              name: "cleanupStepsDone",
              value: [context.workspaceWorktreePath, context.attemptThreadId].filter(Boolean)
                .length,
            },
          ],
          startedAt: updatedAt,
    ...
    startAttemptSession: (input) =>
      Effect.gen(function* () {
        ...
        while (true) {
          ...
          if (!attemptRow) {
            return yield* Effect.fail(
              deps.presenceError(`Attempt '${input.attemptId}' not found.`),
            );
          }
          if (
          ...
          const existingAttemptSelection =
            attemptRow.attemptProvider && attemptRow.attemptModel
              ? ({
                  provider: attemptRow.attemptProvider,
                  model: attemptRow.attemptModel,
                } as ModelSelection)
              : null;
          const selection =
          ...
          `;
            ...
            shouldBootstrapClaimedThread = Boolean(
              existingThread && !existingThread.latestTurn && !latestWorkerHandoff,
            );
        ...
        `;
              ...
              .pipe(
                Effect.catch((cause) =>
                  ...
                  `.pipe(Effect.catch(() => Effect.void));
                    return yield* Effect.fail(
                      deps.presenceError(
                        "Failed to create the worker thread for this attempt.",
                        cause,
                      ),
                    );
                  }),
          ...
          yield* deps.upsertPresenceThreadCorrelation({
            threadId: threadId.toString(),
            boardId: attemptRow.boardId,
            role: "worker",
            ticketId: attemptRow.ticketId,
            attemptId: input.attemptId,
            source: threadCorrelationSource(
              shouldBootstrapWorker ? "attempt_session_created" : "attempt_session_reused",
            ),
          });
      ...
      `;
        ...
        if (context) {
          yield* deps.upsertPresenceThreadCorrelation({
            ...
            ticketId: context.ticketId,
            attemptId: input.attemptId,
            source: threadCorrelationSource("attempt_thread_attached"),
          });
        }
      ...
      Effect.gen(function* () {
        ...
        if (attemptContext) {
          const reportSummary =
            ...
            input.nextStep ??
            (input.blockers.length > 0 ? "Worker reported blockers." : "Worker reported progress.");
          yield* deps
            .writeMissionEvent({
    ...
    resolveFinding: (input) =>
      Effect.gen(function* () {
        ...
        Effect.catch((cause) =>
          Effect.fail(deps.presenceError("Failed to resolve finding.", cause)),
        ),
      ),
    ...
    dismissFinding: (input) =>
      Effect.gen(function* () {
        ...
        Effect.catch((cause) =>
          Effect.fail(deps.presenceError("Failed to dismiss finding.", cause)),
        ),
      ),
    synthesizeWorkerHandoffFromThread,
  };
