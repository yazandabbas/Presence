...
describe("serverSettings helpers", () => {
  ...
  it("replaces and clears the Presence model selection", () => {
    const current = {
      ...DEFAULT_SERVER_SETTINGS,
      presence: {
    ...
    expect(replaced.presence.modelSelection).toEqual({
      provider: "opencode",
      model: "openai/gpt-5",
    });

    expect(
      applyServerSettingsPatch(replaced, {
        presence: {
          modelSelection: null,
        },
      }).presence.modelSelection,
    ).toBeNull();
  });
